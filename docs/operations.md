...
The optional [self-update supervisor](self-update.md) deploys verified commits from the configured main branch after checking CI, local regression tests, service idleness and Slack startup health. It runs independently of Capo and restores the prior launch configuration on startup failure. It does not grant publication or merge approval. Private state stays outside release checkouts; updater/dependency/storage changes require manual deployment.
