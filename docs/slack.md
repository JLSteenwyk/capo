...
2. Install it to the workspace and invite it to #capo. The bot uses `app_mentions:read` and `chat:write` for the interface, plus `channels:read` and `groups:read` to resolve the configured public or private channel during setup. It also uses `channels:history` with `message.channels` for untagged replies in public channels, and `groups:history` with `message.groups` for private channels. Every new message or thread reply from the configured owner in the dedicated channel is addressed to Capo; no @mention is required.
...
## Upgrade an existing Slack app for messages without mentions
