...
A `cancelled.json` marker in a private execution directory stops research before another step. This is an internal cancellation primitive; user-facing cancellation and proactive task execution must additionally enforce the current task state and owner authorization. Recovery itself grants no authority to monitor accounts or perform external work.
