...
Draft saves read back Gmail's raw draft before reporting success. A private verification snapshot records the intended recipients, subject, body and attachment content hashes, reply headers, action marker, and any existing thread identity. Verification tolerates MIME boundary changes, text line-ending normalization, and Google's rewritten Message-ID while requiring attachment bytes to match. Missing or conflicting content keeps the operation pending. Recovery uses the same snapshot after a restart; an action marker alone cannot establish that the complete draft is correct. Older pending saves without a snapshot remain unconfirmed and can be inspected with the draft read tool. Historical completed receipts remain unchanged.

Draft deletion also checks for absence immediately after Google's write response. Only an explicit not-found result produces a verified deletion receipt. A remaining draft, unavailable service, or inconclusive response keeps the action pending; read-only reconciliation can finish it later without another deletion. Verification establishes the observed state at readback; it cannot prevent subsequent edits in Gmail.
