...
## Time and source dates

Each new shared reasoning step receives the current local time and a separate, fixed request-start timestamp. New owner history records retain the Slack message timestamp in UTC; configured timezone information accompanies the request. Previously saved records remain unchanged. A message timestamp dates the message, not an attached screenshot or quoted email. The agent must resolve relative dates from the relevant source, inspect context or research missing dates, and ask only when a material ambiguity remains.

An already-running provider attempt keeps its exact saved prompt for recovery. If its local date becomes stale before it returns, the host discards that decision without executing its tool or delivering its proposed answer. A new step receives fresh time and the original source context. Revalidation consumes the existing attempt budget; exhaustion returns a partial result instead of resetting the budget. Same-day steps refresh time when they begin; this does not guarantee that every time-sensitive fact stays fresh during a long individual step. Model-driven source-date interpretation still needs evaluation.

## Corrections during execution

For ordinary shared Slack requests, the host checks accepted owner messages in the same thread before reasoning, after a provider returns, and immediately before a tool call. A newer instruction supersedes the older request: its unused decision is discarded, completed and uncertain receipts remain saved, and the queued owner follow-up runs with the original thread objective and updated context. The older response briefly acknowledges the update instead of presenting a stale final answer. Each owner message remains a distinct durable request; superseding does not reset the older request's budget.

Explicit `help`, `status`, `details`, and `sync` commands do not supersede work. Other threads, other owners, retrieved content, and model notes cannot trigger this mechanism. Free-form status questions are currently handled as follow-ups. An external operation already sent before a correction arrives cannot be retroactively prevented; its receipt remains available for the next request to inspect and reconcile. Coding and browser workers retain their separate follow-up and cancellation mechanisms.

## Completion reports

The shared loop asks the chief to inventory each requested result in its final structured response: answer, action, or handoff; complete, partial, or needs input; supporting receipt indexes; and a next step for unfinished work. The host rejects nonexistent evidence references and failed or uncertain receipts offered as proof of completion. Action items require a successful registered mutation or verification receipt. A queued handoff cannot prove that the delegated work finished. Read-only reconciliation tools explicitly declare their verification role; arbitrary source content cannot claim that role.

An invalid report prompts a bounded correction without replaying completed writes. Unfinished items are included in the delivered reply and the report is retained in the private request result. Existing `{}` finish responses remain compatible and are labeled `unassessed`, not verified complete. A valid complete inventory is labeled `reported_complete`: the host checks structural and receipt constraints, but the model still selects the requirements and evidence relationships. This is not proof of semantic completeness. Evaluation of omitted requirements and unsupported answer claims remains necessary.
