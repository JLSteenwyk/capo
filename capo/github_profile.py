...
class GitHubProfile:
    ...
    def tools(self):
        return [ReadTool('github.profile_repositories','Discover owned/collaborator/organization repositories for the configured account, excluding prohibited owners and related forks. Page starts at 1.',object_schema({'page':TEXT}),self.repositories,fresh_for=900),
                ReadTool('github.profile_activity','Read account-wide owned or involved issues/PRs, requested reviews, or notifications. Exclusions are host-enforced. Page starts at 1.',object_schema({'kind':{'type':'string','enum':['owned','involved','reviews','notifications']},'page':TEXT}),self.activity,fresh_for=900),
                ReadTool('github.profile_overview','Inspect the configured account across repositories and related activity. Applies exclusions before returning evidence. Includes a rotating batch of repository CI/PR/issue checks, coverage and gaps. Use this first for a standing profile watch; investigate important findings with other GitHub tools. Does not change GitHub state.',object_schema({}),self.overview,fresh_for=900)]
