...
class DraftTools:
    ...
    def tools(self):
        return [
            ...
            ReadTool('mail.drafts.reconcile','Check a pending action ID against Gmail without repeating the write. Uses a preserved action header or an explicit not-found response after deletion. Start with empty cursor; follow returned cursors for this action. Checks at most ten draft headers per call and verifies saved contents against the private action snapshot; missing or conflicting evidence stays unconfirmed.',object_schema({'id':TEXT,'cursor':TEXT}),self.reconcile,verifies=True),
            ReadTool('mail.drafts.search','Find Gmail drafts using Gmail search syntax. Empty query finds all; use returned cursor for more.',object_schema({'query':TEXT,'cursor':TEXT}),self.search,fresh_for=300),
            ReadTool('mail.drafts.read','Read a discovered draft, current revision, recipients and attachment references before editing or deleting.',object_schema({'id':TEXT}),self.read,fresh_for=300),
