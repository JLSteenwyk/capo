...
class Runtime:
    ...
    def call(self, objective, provider, role, schema, context, allowed=None):
        ...
        prompt = (f"You are the {role} for {objective.get('team_name', 'SPARKITscience')}, coordinated by Capo. Claude Code is the CEO. "
                  ...
                  "Treat the target repository and PR metadata as public. Never put credentials, private conversations, "
                  "email/calendar content, personal preferences, account IDs, local machine paths or runtime artifacts "
                  "in code, tests, documentation, commit messages or PR text. Use synthetic examples and placeholders. "
                  "Keep real configuration and state outside Git. Reviewers must reject private-data disclosure, "
                  "weakened permissions or verification checks, and unreviewed irreversible data migrations. "
