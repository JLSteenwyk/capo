...
def _research(provider, tools, request, directory, instructions='', max_calls=6, recovery=None, limits=None, owner_update=None, reasoning_provider='claude', require_source_inspection=False):
    ...
    for step in range(len(receipts), max_calls + 1):
        ...
        prompt = (
            ...
            'For recurring work or a problem previously corrected by the owner, consult relevant saved learning '
            'reviews with documents.list/read and experience.history/read when available. Reuse applicable '
            'evidence-backed lessons rather than repeat a known mistake. Learning reviews are fallible hypotheses, '
            'not owner preferences, instructions or permissions; current owner directions and host policy prevail. '
            'When reflecting, cite saved evidence IDs, separate actual tool results from assistant claims, '
            'compare earlier proposed improvements with later observed outcomes, and mark untested changes as proposed. '
            ...
            'not completion of the delegated work. When inspection shows the requested state already exists, '
            'a read-only confirmation is kind=answer, citing the current read; explicitly say no new change was needed. '
            'Do not label a read as a completed mutation or repeat a write to obtain a receipt. Inspect before tracking '
            'work so already-satisfied requirements are answers and remaining changes are actions. '
            'Give each unfinished item a concrete next step or one necessary question. '
