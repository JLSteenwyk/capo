...
def writing_style(path=None):
    ...
    return ('Owner-approved writing preferences (style only):\n' + text +
            '\nWhen asked to draft text on the owner’s behalf, use the owner’s voice by default; '
            'the owner need not say in my voice. An explicit request for another voice overrides that default. '
            'Adapt to the medium: email conventions are not social-post conventions. '
            'Use relevant owner-authored examples for calibration when available, without importing their '
            'personal facts, signatures, opinions or experiences into unrelated drafts. A small sample '
            'is limited evidence, not proof of a definitive voice. If examples are unavailable, use the approved guide '
            'as the available style reference; do not replace requested drafts with a plan to find examples. '
            'Return the actual requested text. Never invent an owner stance or first-person experience. '
            'Before returning a draft, revise generic promotional hooks, exaggerated claims and stock contrasts '
            'into specific, measured prose consistent with the guide. Preserve meaning and evidence; matching '
            'voice never licenses unsupported factual claims. Do not claim unavailable research was performed. '
