...
- **GitHub delivery:** issue intake, local PR preparation, explicit draft publication, retry reconciliation, and PR/CI status reads work. Slack supports opt-in routine draft publication. Hourly checks can monitor open issues from configured repositories. Merging and branch cleanup use the explicit finish policy described below; automatic CI-triggered repair is not implemented. Development changes use isolated checkouts.
- **Persistence:** objectives, commitments, checkpoints, action receipts, Slack intake, and artifacts persist. Shared research resumes with bounded retries and explicit fallback settings. Hourly checks can perform scoped delegated work, and owner-configured recurring requests use the shared scheduler. Coding objectives still execute serially. See [follow-through and autonomy](docs/autonomy.md) and [recovery](docs/recovery.md) for configuration and limits.
